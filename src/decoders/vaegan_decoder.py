...
def vaegan_decoder(z, args, reuse=False):
    """Decoder architecture as prescribed in VAE/GAN paper
    (https://arxiv.org/pdf/1512.09300.pdf)
    This will be used to convert the encoding of GIF frames
    as input and generates the next frame as output
    Currently it merely reconstructs a single GIF frame from its encoding

    Arguments:
        z {np.ndarray} -- args.z_dim x 1 shaped latent space
            representation of frame(s)
    ...
    Returns:
        recon_frame -- args.crop_width x args.crop_height x 1 shaped
            np.ndarray which is the reconstructed frame for the given z
    """
