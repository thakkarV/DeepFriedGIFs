...
def vaegan_encoder(x, args, reuse=False):
    ...
    """
    # when compressing multiple frames, we need conv3d and when compressing a single frame, we need conv2d
    ...
    with tf.variable_scope("encoder", reuse=reuse):
        ...
        fc1 = tf.layers.dense(inputs=relu3_flattened,
                              units=2048,
                              activation=None,
                              use_bias=True)
